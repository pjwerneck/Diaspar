...
class TkSelectMixIn:
    ...
    def _select(self,
        ...
        # Em Windows, select falha quando recebe a lista _read vazia
        if not _read:
...
if __name__ == '__main__':
    ...
    bar = ButtonBar(root)
    bar.pack(fill=X)
    bar.add_button(1, text='Button 1')
    bar.add_button(2, text='Button 2')
    bar.add_separator()
    bar.add_button(3, text='Button 3')

    ntb = Notebook(root)

    frame1 = Frame(ntb, relief=RAISED, bd=1)
    
    l1 = LabeledEntry(frame1, text='Entry', bg='white', fill=X, expand=1)
    l1.pack(fill=X)

    lst = ScrolledListbox(frame1, bg='white')
    lst.pack(fill=BOTH, expand=1, side=TOP)

    frame2 = Frame(ntb, relief=RAISED, bd=1)

    mlst = MultiListbox(frame2, [("Column A", 15), ("Column B", 15),
                               ("Column C", 15)], bg='white')
    mlst.pack(fill=BOTH, expand=1, side=TOP)

    data1 = [ x for x in range(100)]; random.shuffle(data1)
    data2 = [ x for x in range(100)]; random.shuffle(data2)
    data3 = [ x for x in range(100)]; random.shuffle(data3)
    
    mlst.insert(END, *zip(data1, data2, data3))

    ntb.set_frames(('Frame 1', frame1), ('Frame 2', frame2))
    
    ntb.pack(expand=1, fill=BOTH)

    st = StatusBar(root, text='This is a statusbar')
    st.pack(fill=X)
